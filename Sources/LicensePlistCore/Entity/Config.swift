...
public struct Config: Sendable {
  ...
  public static let empty = Config(
    githubs: [], manuals: [], excludes: [Exclude](), renames: [:], options: .empty)
  ...
  public init(yaml: String, configBasePath: URL) {
    ...
    let renames =
      value["rename"]?.mapping?.reduce([String: String]()) { sum, e in
        guard let from = e.key.string, let to = e.value.string else { return sum }
        var sum = sum
        sum[from] = to
        return sum
      } ?? [:]
    let manuals = Manual.load(
      value["manual"]?.sequence ?? Node.Sequence(), renames: renames, configBasePath: configBasePath
    )
    ...
    nonVersion.forEach {
      Log.warning(
        "\($0.name) is specified by the depricated format. It will be removed at Version 2."
    ...
    let versioned: [GitHub] = (value["github"]?.sequence ?? Node.Sequence())
      .compactMap {
        guard let map = $0.mapping, let owner = map["owner"]?.string, let name = map["name"]?.string
        else {
          return nil
        }
        return GitHub(
          name: name, nameSpecified: renames[name], owner: owner, version: map["version"]?.string)
      }
    let options: GeneralOptions =
      (value["options"]?.mapping ?? Node.Mapping()).map {
        GeneralOptions.load($0, configBasePath: configBasePath)
      } ?? .empty
    self = Config(
      githubs: versioned + nonVersion, manuals: manuals, excludes: excludes, renames: renames,
      options: options)
  }

  public init(
    githubs: [GitHub], manuals: [Manual], excludes: [String], renames: [String: String],
    options: GeneralOptions
  ) {
    self.init(
      githubs: githubs, manuals: manuals, excludes: excludes.map({ Exclude(name: $0) }),
      renames: renames, options: options)
  }

  public init(
    githubs: [GitHub], manuals: [Manual], excludes: [Exclude], renames: [String: String],
    options: GeneralOptions
  ) {
  ...
  func excluded(cocoaPodsLicense: CocoaPodsLicense) -> Bool {
    for exclude in excludes {
      if matches(testString: cocoaPodsLicense.name, matchString: exclude.name)
        ...
        && matches(
          testString: cocoaPodsLicense.licenseType.rawValue, matchString: exclude.licenseType)
      {
        return true
      }
    }
  ...
  private func matches(testString: String?, matchString: String?) -> Bool {
    ...
    let matches = regex.matches(
      in: testString, options: [], range: NSRange(location: 0, length: testString.count))
  ...
  static func extractRegex(_ text: String) -> String? {
    ...
    if matches.count > 1 {
      Log.warning(
        "\(text) contains multiple regex pattern(sandwitched by `/`), but those are ignored except for first one."
      )
    }
...
extension Config: Equatable {
  public static func == (lhs: Config, rhs: Config) -> Bool {
    return lhs.githubs == rhs.githubs && lhs.manuals == rhs.manuals && lhs.excludes == rhs.excludes
      && lhs.renames == rhs.renames && lhs.options == rhs.options
  }
}
