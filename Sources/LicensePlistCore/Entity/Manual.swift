...
public struct Manual: Sendable, Library {
  ...
  public init(
    name n: String, body: String? = nil, source: String?, nameSpecified: String?, version: String?,
...
extension Manual {
  public static func == (lhs: Manual, rhs: Manual) -> Bool {
    return lhs.name == rhs.name && lhs.nameSpecified == rhs.nameSpecified
      && lhs.version == rhs.version && lhs.source == rhs.source
  }
}
...
extension Manual: CustomStringConvertible {
  public var description: String {
    return
      "name: \(name), source: \(source ?? ""), nameSpecified: \(nameSpecified ?? ""), version: \(version ?? "")"
  }
}

extension Manual {
  public static func load(_ raw: Node.Sequence, renames: [String: String], configBasePath: URL)
    -> [Manual]
  {
